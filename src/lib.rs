    ...
    C: Into<Option<Config>>
{
    ...
    write!(cfg.stdout, "; Project web site: {}", env!("CARGO_PKG_REPOSITORY"))?;
    write!(cfg.stdout, ";")?;
    ...
    for file in &files {
        match compute_crc32(file) {
            Ok(crc32) if cfg.print_basename => {
                ...
                write!(cfg.stdout, "{} {:X}",  AsRef::<Path>::as_ref(&name).display(), crc32)?
            }
            Ok(crc32) => write!(cfg.stdout, "{} {:X}", file.display(), crc32)?,
            Err(err) => {
                success = false;
                write!(cfg.stderr, "cksfv: {}: {}", file.display(), err)?
            }
        }
    }
...
pub fn cksfv<C>(sfv: &Path, workdir: Option<&Path>, config: C) -> Result<bool, IoError>
where
    C: Into<Option<Config>>
{
    // get a default config if none provided.
    ...
    let listing = match File::open(sfv) {
        ...
        Err(err) => {
            write!(cfg.stderr, "cksfv: {}: {}", sfv.display(), err);
            return Ok(false);
        }
    };
        ...
        if !line.starts_with(';') {
            ...
            match compute_crc32(&workdir.join(filename)) {
                Ok(crc32_new) if crc32_new != crc32_old => {
                    success = false;
                    if cfg.quiet {
                        write!(cfg.stdout, "{:<50}different CRC", filename.display())?;
                    } else {
                        write!(cfg.stdout, "cksfv: {}: Has a different CRC", filename.display())?;
                    }
                }
                Err(err) if cfg.quiet => {
                    write!(cfg.stdout, "cksfv: {}: {}", filename.display(), err)?;
                }
                Err(err) => {
                    write!(cfg.stdout, "{:<50}{:<30}", filename.display(), err)?;
                    success = false
                }
                Ok(_) => {
                    write!(cfg.stdout, "{:<50}OK", filename.display())?;
                }
            }
        }
    }
